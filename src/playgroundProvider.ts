...
export class PlaygroundProvider implements vscode.WebviewViewProvider {

    public static readonly viewType = 'code-playground.sidebar';
    ...
    public resolveWebviewView(
        ...
        webviewView.webview.onDidReceiveMessage((data: { type: string; value?: any; input?: string; note?: string }) => {
            switch (data.type) {
                case 'runCode':
    ...
    private _updateWebviewState(editor: vscode.TextEditor | undefined) {
        if (this._view) {
    ...
    private _runCode(input: string) {
        ...
        Executor.run(editor.document.uri, input,
            (data) => {
    ...
    private _saveNote(note: string) {
        // Save to global state
        this._context.globalState.update('code-playground.note', note);
    }

    private _sendInitialState() {
        const note = this._context.globalState.get<string>('code-playground.note', '');
